...
def face_recon(FILE_NAME,pwd_SQL):
    ...
    while True:
        ...
        # for index,i in enumerate(frame_list):
        np.save('video_np/'+str(frame_count),frame)
    ...
    out = cv2.VideoWriter('static/video/output_temp_'+save_id+'.mp4',fourcc, fps , (frame_width,frame_height))
    for frame in np.arange(1,frame_count+1,1):
        
        frame2=np.load('video_np/'+str(frame)+".npy")
        
        out.write(frame2)
    
    out.release()
    os.system("ffmpeg -i static/video/output_temp_"+save_id+".mp4 -vcodec libx264 static/video/final_"+save_id+".mp4 -y")
    
    print("Video saved")
    files = glob.glob('./video_np/*')
    for f in files:
        os.remove(f)
